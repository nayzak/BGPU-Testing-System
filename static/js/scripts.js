(function() {
  ...
  $(function() {
    ...
    $('form button#buttonAdd').live('click', function() {
      $('form select#questions_list option:selected').each(function() {
        var not_append, select, value;
        select = $('form select#made_list');
        value = $(this).attr('value');
        not_append = 1;
        $('form select#made_list option').each(function() {
          if ($(this).attr('value') === value) return not_append = 0;
        });
        if (not_append) {
          return select.append('<option value="' + $(this).attr('value') + '">' + $(this).text() + '</option>');
        }
      });
      return false;
    });
    $('form button#buttonDelete').live('click', function() {
      $('form select#made_list option:selected').each(function() {
        return $(this).remove();
      });
      return false;
    });
    return $('form input#submit').live('click', function() {
      $('form select#made_list option').each(function() {
